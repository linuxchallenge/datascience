...
#You can write a command to find the maximum value of Ocollection, Wcollection, Fwcollecion and Tcollection using sapply
	Ocollection <-  sapply(4, function(x) max(bollywood[,x], na.rm = TRUE))
	wcollection <-  sapply(5, function(x) max(bollywood[,x], na.rm = TRUE))
	Fwollection <-  sapply(6, function(x) max(bollywood[,x], na.rm = TRUE))
	Tcollection <-  sapply(7, function(x) max(bollywood[,x], na.rm = TRUE))
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- bollywood$Movie[sapply(4:7, function(x) which.max(bollywood[,x]))]
